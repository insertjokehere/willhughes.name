...
RUN wget https://github.com/spf13/hugo/releases/download/v${HUGO_VER}/hugo_${HUGO_VER}_linux-64bit.tgz && tar -xf hugo_${HUGO_VER}_linux-64bit.tgz && mv hugo_${HUGO_VER}_linux-64bit/hugo_${HUGO_VER}_linux-64bit /usr/local/bin/hugo && rm hugo_${HUGO_VER}_linux-64bit.tgz && rm -r hugo_${HUGO_VER}_linux-64bit
